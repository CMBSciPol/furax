...
class AbstractReader(ABC):
    ...
    def __init__(
        ...
        **keywords: Sequence[Any],
        ...
        self.common_structure = self._get_common_structure(structures)
    ...
    @jax.jit
    def read(self, data_index: int) -> tuple[PyTree[jax.ShapeDtypeStruct], PyTree[Array]]:
        ...
        data = jax.lax.switch(
            data_index,
            [
                lambda i=i: io_callback(callback, self.common_structure, i)
                for i in range(self.count)
            ],
        )
