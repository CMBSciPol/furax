...
class ObservationReader[T](AbstractReader):
    ...
    def _gather_shapes(
        ...
        counts = np.asarray(mhu.process_allgather(np.array([len(local_rows)], np.int32)))
        if (missing := int(counts.max()) - len(local_rows)) > 0:
            local_rows = np.concatenate([local_rows, np.repeat(local_rows[-1:], missing, axis=0)])
