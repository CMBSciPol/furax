...
def gap_fill(
    ...
    *,
    ...
    For a few gaps wider than the correlation length the bare :math:`M_b N^{-1} M_b` system is
    ill-conditioned; passing the flagged-block covariance :math:`M_b N M_b` as ``preconditioner``,
    i.e. ``mask.complement() @ cov @ mask.complement()`` with ``cov`` a cheap approximation of
    :math:`N` (its banded/Fourier form), then helps a lot. Its product with the system operator
    differs from the identity by a boundary term of rank :math:`\approx 2\times` (correlation
    bandwidth) :math:`\times` (number of gaps), and CG converges in roughly that many steps.
