...
def make_two_level_preconditioner(
    ...
    """
    # Extract approximate eigenpairs using Krylov-Schur
    key = key or jax.random.key(186591)
    theta, Z = low_rank(A, rank, method='ks', which='smallest', key=key)
    ...
    # R = I - A @ Q
    id = IdentityOperator(in_structure=A.out_structure)
    R = id - A @ Q
