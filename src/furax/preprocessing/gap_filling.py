...
class GapFillingOperator:
    ...
    def __call__(self, key: PRNGKeyArray, x: Float[Array, ' *shape']) -> Float[Array, ' *shape']:
        ...
        xi = generate_noise_realization(key, x.shape, self.cov, self.detectors, fsamp=self.rate)
...
@partial(jax.jit, static_argnames=('shape', 'detectors'))
def generate_noise_realization(
    ...
    Args:
        key: A PRNG key generated by the user.
        shape: The shape of the output array. The last dimension is the time dimension.
    ...
    if detectors is None:
        ...
        assert detectors.shape == shape[:-1]
        subkeys = detectors.split_key(key)
    real: Float[Array, ' *shape'] = func(psd, subkeys)
    return real
