...
def test_reshape(
    ...
    op = ReshapeOperator(shape=shape, in_structure=as_structure(x))
    if do_jit:
...
def test_reduction(shape: tuple[int, ...], in_structure: PyTree[jax.ShapeDtypeStruct]) -> None:
    op = ReshapeOperator(shape=shape, in_structure=in_structure)
...
def test_invalid_shape1(shape: tuple[int, ...]) -> None:
    ...
    with pytest.raises(ValueError, match='invalid new shape'):
        _ = ReshapeOperator(shape=shape, in_structure=structure)
...
def test_invalid_shape3() -> None:
    ...
    with pytest.raises(ValueError, match='can only specify one unknown dimension'):
        _ = ReshapeOperator(shape=(-1, 2, -1), in_structure=structure)
...
def test_reshape_rules1() -> None:
    op = ReshapeOperator(shape=(-1,), in_structure=jax.ShapeDtypeStruct((10, 20), jnp.float64))
