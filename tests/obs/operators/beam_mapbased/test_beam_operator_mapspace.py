...
def setup_beam_operator_test():
    """Set up data for testing the MapSpaceBeamOperator."""
    n_freq = 2
    n_side = 16
    n_pix = 12 * n_side**2
...
def test_MapSpaceBeamOperator_transpose():
...
def test_MapSpaceBeamOperator_inverse():
...
def test_stokes_to_list_operator_mv():
    ...
    operator = StokesToListOperator(
        axis=setup_data['axis'], in_structure=setup_data['in_structure']
    )
...
def setup_list_to_stokes_test():
    ...
    stokes_list = [
        StokesIQU(
            i=jnp.ones(n_pix) * (i + 1), q=jnp.ones(n_pix) * (i + 2), u=jnp.ones(n_pix) * (i + 3)
        )
...
def test_stacked_beam_operator():
    """Test StackedBeamOperator with multiple MapSpaceBeamOperator instances."""
    d, B, A = setup_beam_operator_test()
    ...
    StackedOperator = StackedBeamOperator(beam_operators=beam_operators, in_structure=d.structure)
...
def test_stacked_beam_operator_transpose():
    """Test transpose of StackedBeamOperator with multiple MapSpaceBeamOperator instances."""
    d, B, A = setup_beam_operator_test()
...
def test_stacked_beam_operator_inverse():
    """Test inverse of StackedBeamOperator with multiple MapSpaceBeamOperator instances."""
    d, B, A = setup_beam_operator_test()
