...
def _create_component(
    ...
    """
    Create a linear operator component for the given astrophysical signal.
    ...
    frequency0 : float
        Reference frequency. For dust, this is dust_nu0; for synchrotron,
        synchrotron_nu0.
    params : PyTree[Array]
        Dictionary containing the spectral parameters, e.g., 'temp_dust',
        'beta_dust', or 'beta_pl'.
    patch_indices : PyTree[Array]
        Dictionary containing the patch indices for spatially varying
        parameters.
    in_structure : Stokes
        The input structure (e.g., a Stokes object) defining the shape and
        configuration.
...
def _get_available_components(params: PyTree[Array]) -> list[str]:
    """
    Determine available astrophysical components from parameters.

    Parameters
    ----------
    params : PyTree[Array]
        Dictionary containing spectral parameters. Expected keys include
        'temp_dust', 'beta_dust', and/or 'beta_pl'.

    Returns
    -------
    list[str]
        List of available components. 'cmb' is always included; 'dust' is
        added if both 'temp_dust' and 'beta_dust' are provided;
        'synchrotron' is added if 'beta_pl' is provided.

    Raises
    ------
    AssertionError
        If only one of 'temp_dust' or 'beta_dust' is provided without the
        other.
    """
...
def _spectral_likelihood_core_bma(
    ...
    """
    Compute base spectral log likelihood components using BMA operator.
    ...
    $$
    \\left(BMA^T N^{-1} d\\right)^T \\left(BMA^T N^{-1} BMA\\right)^{-1}
    \\left(BMA^T N^{-1} d\\right)
    $$

    where:
      - $BMA$ is the complete operator (Bandpass @ HWP @ Mixing matrix @
    ...
    BMA : AbstractLinearOperator
        The complete BMA operator (includes mixing matrix, HWP, bandpass,
        etc.).
    ...
    tuple[SpecParamType, SpecParamType]
        A tuple containing:
          ...
          - s: The solution vector (BMA^T N^{-1} BMA)^{-1}
            (BMA^T N^{-1} d).

    Raises
    ------
    AssertionError
        If provided keys in params or patch_indices are not within the
        valid sets.
    """
    if N_2 is None:
        N_2 = N
...
def sky_signal_bma(
    params: PyTree[Array],
    nu: Array,  # noqa: ARG001
    N: AbstractLinearOperator,
    d: Stokes,
    dust_nu0: float,  # noqa: ARG001
    synchrotron_nu0: float,  # noqa: ARG001
    BMA: AbstractLinearOperator,
    patch_indices: PyTree[Array] = single_cluster_indices,
    N_2: AbstractLinearOperator | None = None,
) -> ComponentParametersDict:
    """
    Compute the estimated sky signal based on BMA operator.

    This function extracts the sky vector 's' from the base spectral log
    likelihood computation using the BMA operator, which represents the
    reconstructed sky signal.
    ...
    patch_indices : PyTree[Array], optional
        Patch indices for spatially varying parameters (default is
        single_cluster_indices).
    N_2 : AbstractLinearOperator or None
        Optional secondary noise operator; if None, it defaults to N.
...
def spectral_log_likelihood_bma(
    ...
    """
    Compute the spectral log likelihood using BMA operator.

    The likelihood is calculated based on the weighted data vector and its
    associated solution, using the complete BMA operator instead of just
    the mixing matrix.
...
def negative_log_likelihood_bma(
    ...
    """
    Compute the negative spectral log likelihood using BMA operator.

    This function returns the negative of the spectral log likelihood using
    the complete BMA operator, which is useful for optimization procedures
    where minimizing the negative log likelihood is equivalent to
...
def sky_signal(
    ...
    """
    Compute the estimated sky signal based on spectral parameters.

    This function extracts the sky vector 's' from the base spectral log
    likelihood computation, which represents the reconstructed sky signal.
