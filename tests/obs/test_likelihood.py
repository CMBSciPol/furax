...
if TYPE_CHECKING:
    import furax.core._blocks
    import furax.obs.stokes
import jax
...
from furax.obs.operators import (
    MixingMatrixOperator, CMBOperator, DustOperator,
    SynchrotronOperator, NoiseDiagonalOperator
)
...
def test_base_case(likelihood_setup):
    ...
    with jax.disable_jit():
        AND, s = _base_spectral_log_likelihood(
            base_params, patch_indices, nu, N1, d, dust_nu0, synchrotron_nu0,
            op=None, N_2=None
        )
...
def test_dust_only(likelihood_setup):
    ...
    with jax.disable_jit():
        AND, s = _base_spectral_log_likelihood(
            dust_params, patch_indices, nu, N1, d, dust_nu0, synchrotron_nu0,
            op=None, N_2=None
        )
...
def test_synchrotron_only(likelihood_setup):
    ...
    with jax.disable_jit():
        AND, s = _base_spectral_log_likelihood(
            synchrotron_params, patch_indices, nu, N1, d, dust_nu0, synchrotron_nu0,
            op=None, N_2=None
        )
